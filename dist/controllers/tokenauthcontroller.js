...
exports.generateToken = function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            const accessToken = yield jwtTool.generateToken(userFakeData, jwtConfig.accessTokenSecret, jwtConfig.accessTokenLife);
            // debug(`Thực hiện tạo mã Refresh Token, [thời gian sống 10 năm] =))`);
            const refreshToken = yield jwtTool.generateToken(userFakeData, jwtConfig.refreshTokenSecret, jwtConfig.refreshTokenLife);
            // Lưu lại 2 mã access & Refresh token, với key chính là cái refreshToken để đảm bảo unique và không sợ hacker sửa đổi dữ liệu truyền lên.
            // save to database
            const result = yield Database.sequelize.query(databasequery.addtokenlist(accessToken, refreshToken, new Date()), { type: QueryTypes.INSERT });
            // save fail
            if (!result)
                if (result.length < 2) /*accessToken, refreshToken = 2*/
                    return;
...
exports.refreshToken = function (req, res) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const result = yield Database.sequelize.query(databasequery.checkexistsaccesstoken(refreshTokenFromClient), { type: QueryTypes.SELECT });
        // tslint:disable-next-line:no-console
        ...
        if (refreshTokenFromClient && tokenExists > 0) {
            try {
                ...
                const decoded = yield jwtTool.verifyToken(refreshTokenFromClient, jwtConfig.refreshTokenSecret);
                ...
                const accessToken = yield jwtTool.generateToken(userFakeData, jwtConfig.accessTokenSecret, jwtConfig.accessTokenLife);
                const update = yield Database.sequelize.query(databasequery.updateaccesstoken(accessToken, refreshTokenFromClient), { type: QueryTypes.UPDATE });
                // tslint:disable-next-line:no-console
